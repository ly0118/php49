...
<div class="table-box">
	<table>
		...
		<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr> 
				...
				<?php if(!empty($vol["file"])): ?><a href = 'javascript:;' class="download" data="<?php echo ($vol["id"]); ?>">【下载】</a><?php endif; ?>
				</td>
                <td class="content"><?php echo (msubstr($vol["content"],0,20)); ?></td>
                <td class="addtime"><?php echo (date('Y-m-d H:i:s',$vol["addtime"])); ?></td>
                <td class="status"><?php if($vol["isread"] == 0): ?><span style="color:red">未读</span><?php else: ?><span style="color:blue">已读</span><?php endif; ?></td>
                <td class="operate">
...
$(function(){
	...
	$('.show').on('click',function(){
		...
		layer.open({
            ...
            shade: 0.8,  
            area: ['450px', '90%'],
            content: '/index.php/Admin/Email/getContent/file/1/id/'+id, //iframe的url
			end:function(){
				if(emailIsread == 0){
